...
dsgnhsf kgj mfm vjmmf wptc rtb dtblv frp wwmngbk dxss txz fzlxll lbqjrp hhcxnnhnskhcwxw 
...
for n in range(qtdA):	
	...
	if (palavra[comp-1:comp] not in 'z,m,b') and  (palavra[0:1] in 'z,m,b'):
		quantidadeA += 1
...
for n in range(qtdB):	
	...
	if (palavra[comp-1:comp] not in TipoZumbi) and  (comp == 7):
		quantidadeB += 1
		if (palavra[0:1] not in TipoZumbi):
			quantidadeBP += 1

print 'B; %d' % (quantidadeB)
print 'C; %d' % (quantidadeBP)
